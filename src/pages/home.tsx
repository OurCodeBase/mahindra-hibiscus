  ...
  return (
    <div className="relative flex justify-center items-center">
      <label htmlFor="checkbox" className="relative flex size-8 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-tr from-[#4158D0] via-[#C850C0] to-[#FFCC70] p-2 duration-100 hover:p-2.5">
        ...
        <label htmlFor="checkbox" className="size-full rounded-md bg-background peer-checked:size-0"></label>
        ...
        ></div>
        <div className="absolute left-[0.4rem] top-4 h-[4px] w-[10px] -translate-x-10 -translate-y-10 rotate-[45deg] rounded-sm bg-white duration-300 peer-checked:translate-x-0 peer-checked:translate-y-0"></div>
      </label>
      <span className="ml-2 flex flex-row w-[1px]">Check Commercials</span>
    </div>
  )
}
...
export default function App() {
  ...
  return <>
    ...
    <Logo/>
      <form onSubmit={handleSearch} className="flex py-10 flex-row w-full justify-center">
        <input
          type="text"
          name="vinnos"
          placeholder="Feed me vin no. column"
          className="px-4 bg-green-50 text-green-600 placeholder-stone-400 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-contessa focus:ring-2 focus:ring-contessa-dim transition-all cursor-pointer"
        />
        <button
          type="submit"
